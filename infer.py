...
cmd = "--config_name emotion --inputter_name emotion --add_nlg_eval --seed 0 --load_checkpoint /mnt/storage/chuyuqi/code/DATA/emotion.emotion/2024-04-10212949.1e-05.16.1gpu/epoch-2.bin --fp16 false --max_input_length 150 --max_decoder_input_length 50 --max_length 50 --min_length 10 --infer_batch_size 2 --infer_input_file /mnt/storage/chuyuqi/code/data_lama/test_lama.txt --temperature 0.8 --top_k 0 --top_p 0.9 --num_beams 1 --repetition_penalty 1 --no_repeat_ngram_size 0 "
...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    for batch, posts, references, sample_ids in infer_dataloader:
        ...
        # print("batch_other_res:", batch_other_res)
        # asdfdsfsd
        ...
        for idx in range(len(sample_ids)):
            ...
            # print(other_res_to_append)

            tmp_res_to_append.update(other_res_to_append)
            # print(tmp_res_to_append)
            # asfdsfsdfsds
    ...
    save_file_path = os.path.join(save_dir, 'gen.json')
    ...
    if not args.only_generate:
        ...
        if 'acc' in other_res:
            ...
            pred_emotion_id = other_res["acc"]["pred_user_id"] + other_res["acc"]["pred_sys_id"]
            cls_emotion_id = other_res["acc"]["cls_user_id"] + other_res["acc"]["cls_sys_id"]

            print(f'emotion classification_report\n', classification_report(cls_emotion_id, pred_emotion_id))
            print(f'emotion confusion_matrix\n', confusion_matrix(cls_emotion_id, pred_emotion_id))
            ...
            for k, v in other_res['acc_map'].items():
                if k not in other_res['acc'] or v not in other_res['acc']: # TODO
                    print("skip acc_map key: {} value: {}".format(k, v))
                    continue # TODO
