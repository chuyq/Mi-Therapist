...
def featurize(
    ...
    response, max_decoder_input_length, user_emotion_id,strat_id, sys_emotion_id
):
    context = [c + [eos] for c in context]
    input_ids = sum(context, [])[:-1]
    input_ids = input_ids[-max_input_length:]

    initial_labels = [user_emotion_id] + sep_emotion + [strat_id] + [sys_emotion_id] + response
    # print("initial_labels is", initial_labels)
    # asdfdsfd

    if len(initial_labels) >= max_decoder_input_length:
        # If the labels exceed or meet the max length, slice to max_decoder_input_length - 1 to leave space for eos
        labels = initial_labels[:max_decoder_input_length - 1] + [eos]
    else:
        # If there's enough space, simply append eos at the end
        labels = initial_labels + [eos]
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, **kwargs):
    ...
    inputs = []
    context = []
    last_user_emotion_id = None
    
    for i in range(len(dialog)):
        if dialog[i]['text'] is None:
            print(data['dialog'])
        else:
        ...
        if dialog[i]['speaker'] == 'user':
            user_emotion_id = process('[' + dialog[i]['emotion'] + ']')
            assert len(user_emotion_id) == 1
            last_user_emotion_id = user_emotion_id[0]

            # print("user_emotion_id:",user_emotion_id )
        
        if dialog[i]['speaker'] == 'sys':
            strat_id = process('[' + dialog[i]['strategy'] + ']')

            try:
            # 检查 strat_id 是否只包含一个元素
                assert len(strat_id) == 1
            except AssertionError:
            # 如果 strat_id 的长度不是 1，打印错误信息和相关对话
                print(f"Error: strat_id length is not 1 for dialog index {i} with strategy '{dialog[i]['strategy']}'")
                print(dialog[i]['text'])
            ...
            sys_emotion_id = process('[' +'sys_' + dialog[i]['emotion'] + ']')

            assert len(sys_emotion_id) == 1
            sys_emotion_id = sys_emotion_id[0]

            # print("sys_emotion_id:", sys_emotion_id)

            if last_user_emotion_id is not None:
                user_emotion_id = last_user_emotion_id

            else:
                user_emotion_id = process("[depression]")
                assert len(user_emotion_id) == 1
                user_emotion_id = user_emotion_id[0]  
...
def convert_inputs_to_features(inputs, toker, **kwargs):
    ...
    sep_emotion = process("[sep_emotion]")  #54961
    ...
    for i in range(len(inputs)):
        ...
        feat = featurize(
            ...
            ipt['response'], max_decoder_input_length, ipt['user_emotion_id'], ipt['strat_id'], ipt['sys_emotion_id']
        )
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        for f in features:
            sep_position = f.labels.index(sep_emotion)
            strat_position = sep_position + 1
            strat_id_value = f.labels[strat_position]-54944
            strat_id.append(strat_id_value)
            user_position = sep_position -1
            user_id_value = f.labels[user_position]-54962
            user_id.append(user_id_value)
            sys_position = sep_position +2
            sys_id_value = f.labels[sys_position]-54954
            sys_id.append(sys_id_value)


        strat_id = torch.tensor(strat_id, dtype=torch.long)
        # print("strat_id:",strat_id)
        user_id = torch.tensor(user_id, dtype=torch.long)
        # print("user_id:",user_id)
        sys_id = torch.tensor(sys_id, dtype=torch.long)
        # print("sys_id:",sys_id)
...
def prepare_infer_batch(features, toker, interact=None):
    ...
    other_res['acc_map'] = {
        ...
        'cls_user_id': 'pred_user_id',
        'cls_sys_id': 'pred_sys_id',
    }

    # other_res['acc_emotion'] = {
    #     'cls_user_id': 'pred_user_id',
    #     'cls_sys_id': 'pred_sys_id',
    # }
    ...
    if interact is None and GOLDEN_TRUTH:
        other_res['cls_user_id'] = res.get('user_id')
    else:
        other_res['cls_user_id'] = res.pop('user_id')

    if interact is None and GOLDEN_TRUTH:
        other_res['cls_sys_id'] = res.get('sys_id')
    else:
        other_res['cls_sys_id'] = res.pop('sys_id')
